...
import com.javagamedev.utility.LoopingByteInputStream;
import com.javagamedev.utility.ThreadPool;
...
public class SoundManager extends ThreadPool {
	...
	*/
	public Sound getSound(AudioInputStream audioStream) {
	    ...
	    if (frames == AudioSystem.NOT_SPECIFIED || frameSize <= 0) {
	    	try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
	            byte[] buffer = new byte[4096];
	            int n;
	            while ((n = audioStream.read(buffer)) != -1) {
	        ...
	        int length = (int)(frames * frameSize);
