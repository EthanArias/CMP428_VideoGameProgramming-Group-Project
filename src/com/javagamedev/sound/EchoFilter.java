...
public class EchoFilter extends SoundFilter {
	 ...
	 * the source. A decay value of .5 means the echo heard is half as loud as the source.
	 */
	 ...
	 * Ensures that the sound will decayed to below 1% 
	 * of maximum value (amplitude).
	 */
	...
	public void filter(byte[] samples, int offset, int length) {
		for (int i=offset; i<offset+length; i+=2) {
			// update the sample
			short oldSample = getSamples(samples, i);
